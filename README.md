...
| `implement_interface` | Generate interface member implementations for a type (indexers emit `this[...]` indexer declarations via `CreateIndexerStub`; ordinary properties stay on `CreatePropertyStub`). | `sourceFile`, `typeName`, `interfaceName`, `explicitImplementation`, `members` |
