...
| `encapsulate_field` | Convert a field to a property with backing field (`updateReferences`: default true rewrites external references to the new property; false still encapsulates — private field + property — and does not redirect callers to the property; if the backing field is renamed for a case-only collision (`name` → `_name`), external identifiers follow the new field name; same-class references stay on the field; `preview` describes whether references will be updated and writes nothing). | `sourceFile`, `fieldName`, `propertyName`, `readOnly`, `updateReferences`, `preview` |
