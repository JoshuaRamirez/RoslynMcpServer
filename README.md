...
| `generate_tostring` | Generate a ToString() override for a type (`format`: interpolated or stringbuilder; `includeInheritedMembers`: default false keeps this-type-only collection, true also collects accessible instance fields/properties declared on base types; `replaceExisting`: replace an existing parameterless ToString instead of failing). | `sourceFile`, `typeName`, `fields`, `format`, `includeInheritedMembers`, `replaceExisting` |
