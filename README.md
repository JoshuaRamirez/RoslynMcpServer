...
| `convert_foreach_linq` | Convert foreach loops with Add patterns to LINQ (`preferQuerySyntax`: default false keeps today's method syntax `.Where().Select().ToList()`; true emits query syntax (`from … where … select`) for filter / project / filter+project / ToList-after-query; Any / All / FirstOrDefault / Count-only keep method syntax rather than inventing invalid query syntax; `column` picks the foreach whose keyword covers that column on the given line; `preview` describes the rewrite and writes nothing). | `sourceFile`, `line`, `column`, `preferQuerySyntax`, `preview` |
