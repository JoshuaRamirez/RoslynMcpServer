...
| Tool | Description | Key Parameters |
|------|-------------|----------------|
| `inline_method` | Inline a method by replacing call sites with the method body. Optionally remove the method (`column` picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName + optional line identifier start-line pick so a continuation-line identifier still matches when `line` is the identifier line of a split signature). | `sourceFile`, `methodName`, `line`, `column`, `callSiteLocation`, `removeMethod` |
