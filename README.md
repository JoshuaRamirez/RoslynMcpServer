...
| Tool | Description | Key Parameters |
|------|-------------|----------------|
| `generate_equals_hashcode` | Generate Equals() and GetHashCode() overrides for a type based on its fields/properties (`implementIEquatable`: also add IEquatable&lt;T&gt; and typed Equals; `generateOperators`: also add `==` / `!=`; `replaceExisting`: replace existing equality members instead of failing; `useHashCodeCombine`: default true uses HashCode.Combine / builder, false uses unchecked prime-multiply). | `sourceFile`, `typeName`, `fields`, `implementIEquatable`, `generateOperators`, `replaceExisting`, `useHashCodeCombine` |
