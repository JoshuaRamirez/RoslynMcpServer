...
| `extract_base_class` | Extract members (methods, properties, fields, and events) to a new base class. Field-like and accessor-style events move by name; a multi-variable event field moves only the selected declarator. Private events become protected on the new base. `makeAbstract` still only marks the new class abstract. | `sourceFile`, `typeName`, `baseClassName`, `members`, `targetFile`, `separateFile`, `makeAbstract` |
