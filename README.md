...
| `add_braces` | Add braces to control statements (`if`, `else`, `for`, `foreach`, `while`, `using`) that have a single-statement body, preserving semantics. Process a single file or all files in the solution. `scope` is `statement` (default; wrap the body at `line` / optional `column`; single-file only), `file` (every braceless control body in the file), or `type` (bodies inside `typeName`; single-file only). `allFiles: true` walks every C# file at file scope and cannot be combined with `scope=statement` or `scope=type`. `else if` stays a single construct in file/type scope. | `sourceFile`, `allFiles`, `line`, `column`, `scope`, `typeName`, `preview` |
