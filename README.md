...
| `pull_members_up` | Move selected members (methods, properties, fields, and events) from a derived type onto an existing base class or interface. Field-like and accessor-style events move by name; a multi-variable event field moves only the selected declarator. Private events become protected on the base. `makeAbstract` on a class target makes the event abstract and keeps an override on the derived type; interface targets keep the implementation on the derived type. | `sourceFile`, `typeName`, `members`, `targetBaseType`, `makeAbstract` |
| `push_members_down` | Move selected members (methods, properties, fields, and events) from a base type down onto derived types. Field-like and accessor-style events move by name; a multi-variable event field moves only the selected declarator. `leaveAbstract` on a class source makes the event abstract on the base and adds an override on each derived type. | `sourceFile`, `typeName`, `members`, `targetDerivedTypes`, `leaveAbstract` |
