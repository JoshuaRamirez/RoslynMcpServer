...
public class HelpGeneratorTests
{
    ...
    public void GenerateToolHelp_IntroduceParameter_ShowsColumn()
    {
        ...
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile optional", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, variableName, and line are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--source-file", optionalSection);
        Assert.Contains("--all-files", optionalSection);
        Assert.Contains("--variable-name", optionalSection);
    ...
    [Fact]
    public void GenerateToolHelp_IntroduceField_ShowsAllFiles()
    {
        var registry = ToolRegistry.BuildDefault();
        var tool = registry.GetTool("introduce-field")!;
        var help = HelpGenerator.GenerateToolHelp(tool);

        Assert.Contains("introduce-field", help);
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile optional", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, startLine, startColumn, endLine, endColumn, and fieldName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--is-readonly", optionalSection);
        Assert.Contains("--is-static", optionalSection);
        Assert.Contains("--initialize-in-constructor", optionalSection);
    ...
    [Fact]
    public void GenerateToolHelp_ExtractMethod_ShowsAllFiles()
    {
        var registry = ToolRegistry.BuildDefault();
        var tool = registry.GetTool("extract-method")!;
        var help = HelpGenerator.GenerateToolHelp(tool);

        Assert.Contains("extract-method", help);
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile optional", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, startLine, startColumn, endLine, endColumn, and methodName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--visibility", optionalSection);
        Assert.Contains("--make-static", optionalSection);
        Assert.Contains("--preview", optionalSection);
    }

    [Fact]
    public void GenerateToolHelp_ExtractVariable_ShowsAllFiles()
    {
        var registry = ToolRegistry.BuildDefault();
        var tool = registry.GetTool("extract-variable")!;
        var help = HelpGenerator.GenerateToolHelp(tool);

        Assert.Contains("extract-variable", help);
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile optional", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, startLine, startColumn, endLine, endColumn, and variableName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--end-column", optionalSection);
        Assert.Contains("--variable-name", optionalSection);
        Assert.Contains("--use-var", optionalSection);
        Assert.Contains("--replace-all", optionalSection);
        Assert.Contains("--preview", optionalSection);
    }
    ...
    public void GenerateToolHelp_ExtractInterface_ShowsSeparateFile()
    {
        ...
        Assert.Contains("--all-files", help);
        ...
        Assert.Contains("FirstOrDefault", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, typeName, and interfaceName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--line", optionalSection);
        Assert.Contains("--column", optionalSection);
        Assert.Contains("--separate-file", optionalSection);
        Assert.Contains("--preview", optionalSection);
    }
    ...
    public void GenerateToolHelp_ExtractBaseClass_ShowsSeparateFile()
    {
        ...
        Assert.Contains("FirstOrDefault", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, typeName, baseClassName, and members are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--all-files", optionalSection);
        Assert.Contains("--type-name", optionalSection);
        Assert.Contains("--base-class-name", optionalSection);
        Assert.Contains("--line", optionalSection);
        Assert.Contains("--column", optionalSection);
        Assert.Contains("--separate-file", optionalSection);
        Assert.Contains("--preview", optionalSection);
    ...
    public void GenerateToolHelp_PullMembersUp_ShowsLine()
    {
        ...
        Assert.Contains("FirstOrDefault", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile, typeName, and members are required", tool.Description, StringComparison.OrdinalIgnoreCase);
    ...
    public void GenerateToolHelp_ChangeSignature_ShowsColumn()
    {
        ...
        // parameters stays required; sourceFile/methodName become optional with allFiles.
    ...
    public void GenerateToolHelp_AddParameter_ShowsColumn()
    {
        ...
        // parameterName / parameterType stay required; sourceFile/methodName become optional with allFiles.
    ...
    [Fact]
    public void GenerateToolHelp_AddParameter_ShowsAllFiles()
    {
        var registry = ToolRegistry.BuildDefault();
        var tool = registry.GetTool("add-parameter")!;
        var help = HelpGenerator.GenerateToolHelp(tool);
        ...
        Assert.Contains("--parameter-type", help);
    }
    ...
    public void GenerateToolHelp_InlineMethod_ShowsColumn()
    {
        ...
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile optional", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile and methodName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
    ...
    public void GenerateToolHelp_RenameNamespace_ShowsColumn()
    {
        ...
        Assert.Contains("sourceFile and namespaceName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Contains("--new-name", requiredSection);
        Assert.DoesNotContain("--source-file", requiredSection);
        Assert.DoesNotContain("--namespace-name", requiredSection);
        Assert.DoesNotContain("--column", requiredSection);
        Assert.DoesNotContain("--line", requiredSection);
        Assert.DoesNotContain("--preview", requiredSection);

        Assert.Contains("--source-file", optionalSection);
        Assert.Contains("--namespace-name", optionalSection);
        Assert.Contains("--line", optionalSection);
        Assert.Contains("--column", optionalSection);
        Assert.Contains("--preview", optionalSection);
    ...
    [Fact]
    public void GenerateToolHelp_RenameNamespace_ShowsAllFiles()
    {
        var registry = ToolRegistry.BuildDefault();
        var tool = registry.GetTool("rename-namespace")!;
        var help = HelpGenerator.GenerateToolHelp(tool);
    ...
    public void GenerateToolHelp_InlineVariable_ShowsColumn()
    {
        ...
        Assert.Contains("allFiles", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile optional", tool.Description, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("sourceFile and variableName are required", tool.Description, StringComparison.OrdinalIgnoreCase);
