...
/// <summary>
/// Operation-level tests for <see cref="GenerateEqualsHashCodeOperation"/>, including optional <c>line</c> / <c>column</c>, <c>implementIEquatable</c>, <c>generateOperators</c>, <c>replaceExisting</c>, <c>useHashCodeCombine</c>, <c>includeProperties</c>, <c>callSuper</c>, <c>includeInheritedMembers</c>, and <c>allFiles</c>.
/// </summary>
public class GenerateEqualsHashCodeOperationTests
{
    ...
    {
        Assert.Equal("Generate Equals/GetHashCode", GenerateEqualsHashCodeOperation.BuildAllFilesDescription(1));
        Assert.Equal("Generate Equals/GetHashCode on 2 types", GenerateEqualsHashCodeOperation.BuildAllFilesDescription(2));
    }

    [SkippableFact]
    public async Task GenerateEquals_AllFilesFalse_GeneratesOnlySpecifiedType()
    {
        ...
        Assert.Contains("public override bool Equals(object?", updatedA, StringComparison.Ordinal);
        Assert.Contains("public override int GetHashCode()", updatedA, StringComparison.Ordinal);
        Assert.DoesNotContain("public override bool Equals(object?", await File.ReadAllTextAsync(workspace.SourcePaths["FileB.cs"]), StringComparison.Ordinal);
    ...
    [SkippableFact]
    public async Task GenerateEquals_OmittedAllFiles_KeepsSingleSiteGenerate()
    {
        ...
        Assert.True(result.Success);
        var updated = NormalizeNewlines(await File.ReadAllTextAsync(workspace.SourcePath));
        Assert.Contains("public override bool Equals(object?", updated, StringComparison.Ordinal);
        Assert.Contains("public override int GetHashCode()", updated, StringComparison.Ordinal);
        Assert.DoesNotContain("ISkip", ExtractMethod(updated, "public override bool Equals(object?"));
    }

    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_GeneratesEligibleTypesAcrossFiles()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
            ("FileA.cs", EligibleFileA),
            ("FileB.cs", EligibleFileB),
        ...
        Assert.Contains("public override bool Equals(object?", updatedA, StringComparison.Ordinal);
        Assert.Contains("other.Name", updatedA, StringComparison.Ordinal);
        Assert.Contains("public override int GetHashCode()", updatedA, StringComparison.Ordinal);
        Assert.Contains("public override bool Equals(object?", updatedB, StringComparison.Ordinal);
        Assert.Contains("other.Age", updatedB, StringComparison.Ordinal);
        Assert.Equal(beforeC, await File.ReadAllTextAsync(workspace.SourcePaths["FileC.cs"]));
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_WithoutSourceFileOrTypeName_Succeeds()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
            ("FileA.cs", EligibleFileA),
            ("FileB.cs", EligibleFileB));
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesFalse_WithoutSourceFile_MissingRequiredParam()
    {
        await using var workspace = await TempWorkspace.CreateAsync(EligibleFileA, "FileA.cs");
        var operation = new GenerateEqualsHashCodeOperation(workspace.Context);
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesFalse_WithoutTypeName_MissingRequiredParam()
    {
        await using var workspace = await TempWorkspace.CreateAsync(EligibleFileA, "FileA.cs");
        var operation = new GenerateEqualsHashCodeOperation(workspace.Context);
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_WithTypeName_Rejects()
    {
        await using var workspace = await TempWorkspace.CreateAsync(EligibleFileA, "FileA.cs");
        var operation = new GenerateEqualsHashCodeOperation(workspace.Context);
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_WithFields_Rejects()
    {
        await using var workspace = await TempWorkspace.CreateAsync(EligibleFileA, "FileA.cs");
        var operation = new GenerateEqualsHashCodeOperation(workspace.Context);
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_WithLine_Rejects()
    {
        await using var workspace = await TempWorkspace.CreateAsync(EligibleFileA, "FileA.cs");
        var operation = new GenerateEqualsHashCodeOperation(workspace.Context);
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_WithColumn_Rejects()
    {
        await using var workspace = await TempWorkspace.CreateAsync(EligibleFileA, "FileA.cs");
        var operation = new GenerateEqualsHashCodeOperation(workspace.Context);
    ...
    [SkippableFact]
    public async Task GenerateEquals_PreviewAllFiles_AggregatesChangedFilesAndWritesNothing()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
            ("FileA.cs", EligibleFileA),
            ("FileB.cs", EligibleFileB),
        ...
        Assert.Contains(result.PendingChanges, c =>
            c.Description.Contains("Equals", StringComparison.OrdinalIgnoreCase) &&
            c.AfterSnippet != null &&
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_EveryFileIneligible_SucceedsWithEmptyChanges()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
            ("FileC.cs", IneligibleFileC),
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_SkipsInterfaceNoMembersAndAlreadyHasOverride()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
        ...
        Assert.True(TypeHasMethod(eligible, "Equals"));
        Assert.True(TypeHasMethod(eligible, "GetHashCode"));
        Assert.True(TypeHasMethod(outer, "Equals"));
        Assert.True(TypeHasMethod(nested, "Equals"));
        Assert.Equal(1, CountOccurrences(NormalizeNewlines(alreadyHas.ToFullString()), "public override bool Equals(object?"));
        Assert.False(TypeHasMethod(noMembers, "Equals"));
        Assert.DoesNotContain("public override bool Equals(object?", updated[updated.IndexOf("public interface ISkip", StringComparison.Ordinal)..updated.IndexOf("public class NoMembers", StringComparison.Ordinal)], StringComparison.Ordinal);
        Assert.Single(result.Changes!.FilesModified);
    }

    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_OptionalSourceFile_LimitsWalk()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
            ("FileA.cs", EligibleFileA),
            ("FileB.cs", EligibleFileB));
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_OptionalSourceFile_MatchesIgnoreCase()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
            ("FileA.cs", EligibleFileA),
            ("FileB.cs", EligibleFileB));
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_ReplaceExisting_ReplacesMatchingEquality()
    {
        await using var workspace = await TempWorkspace.CreateWithFilesAsync(
        ...
        Assert.Equal(1, CountOccurrences(updated, "public override bool Equals(object?"));
        Assert.Contains("other.Name", updated, StringComparison.Ordinal);
        Assert.DoesNotContain("=> false", updated, StringComparison.Ordinal);
    }

    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_SameNamedFileLocalTypes_BothGetStubs()
    {
        const string fileA = """
            namespace TestApp;
    ...
    [SkippableFact]
    public async Task GenerateEquals_AllFilesTrue_GenuinePartial_ImplementedOnce()
    {
        const string partA = """
            namespace TestApp;
        ...
        Assert.Contains("public override bool Equals(object?", updatedA, StringComparison.Ordinal);
        Assert.Equal(1, CountOccurrences(updatedA, "public override bool Equals(object?"));
    ...
    public void CollectTypeDeclarations_IncludesNestedAndInterface()
    {
        ...
        var types = GenerateEqualsHashCodeOperation.CollectTypeDeclarations(root);
