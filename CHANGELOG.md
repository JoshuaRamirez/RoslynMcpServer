...
### Changed
- `generate_tostring` now honors `replaceExisting`: omitted / false keeps today's fail-on-existing behavior (`AlreadyHasOverride` 3056, "Type already has a ToString override.") when a non-implicit parameterless `ToString` already exists; `true` removes existing parameterless instance `ToString()` overrides on the target type across every partial that holds one, then generates a fresh override for the requested `fields` / `format` / `includeInheritedMembers`. Parameterized overloads (`ToString(string)`, `ToString(IFormatProvider)`, etc.) are left alone. Implicit `Object` / `ValueType` `ToString` does not count as existing (already today's rule). Additive with `fields`, `format`, and `includeInheritedMembers`. Preview describes the replacement / generated members and writes nothing. No new error codes for the happy replace path.
