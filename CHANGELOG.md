...
- `change_return_type` — change a method's declared return type and convert return statements when safe (implicit conversions left alone; void↔non-void rewritten), with override/interface updates, preview mode, and rejects for same type, invalid type syntax, incompatible returns, overload collisions, method-group references that would break, missing methods, and uneditable documents
