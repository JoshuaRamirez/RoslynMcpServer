...
### Changed
- `generate_constructor` now honors `replaceExisting`: omitted / false keeps today's `ConstructorExists` rejects (exact signature match, optional-param ambiguity, required-param conflict); `true` removes an existing non-implicit constructor with the **exact** same signature (same parameter count and types in order via `SymbolEqualityComparer`) across every partial that holds it, then inserts the newly generated constructor for the requested `includeProperties` / `includeInheritedMembers` / `members` / `addNullChecks`. Optional-param / required-param **ambiguity** without an exact match still throws `ConstructorExists` — the flag does not guess which overload to replace. When no exact-signature constructor exists, generation proceeds as today (no remove). Implicit constructors are not treated as existing (already today's rule). Additive with `includeProperties`, `includeInheritedMembers`, `members`, `addNullChecks`, and `preview`. Preview with an exact match describes replacement and writes nothing. `MemberNotFound` is unchanged. No visibility or copy-constructor options in this increment.
