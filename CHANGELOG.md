...
### Changed
- `generate_equals_hashcode` now honors `useHashCodeCombine`: omitted / true keeps today's `HashCode.Combine` (≤8 members) or `HashCode` builder (&gt;8 members), qualifying `global::System.HashCode` so a type-local `HashCode` cannot steal the call; `false` emits a classic unchecked prime-multiply `GetHashCode` (seed 17, `hash = (hash * 31) + memberHash`) with `Member.GetHashCode()` for non-nullable value types and `(Member?.GetHashCode() ?? 0)` for nullable reference / nullable value members. Additive with `fields`, `implementIEquatable`, `generateOperators`, and `replaceExisting`. Preview describes the chosen GetHashCode style and writes nothing.
