...
/// honors <c>callSuper</c> to fold the immediate base type's equality into Equals/GetHashCode,
/// and honors optional <c>allFiles</c> to walk every C# document (or the
/// optional single <c>sourceFile</c>) and generate Equals/GetHashCode on
...
public sealed class GenerateEqualsHashCodeOperation : RefactoringOperationBase<GenerateEqualsHashCodeParams>
{
    ...
    /// <inheritdoc />
    protected override void ValidateParams(GenerateEqualsHashCodeParams @params) => Validate(@params);

    /// <summary>
    /// Validates generate-equals-hashcode parameters. Internal so tests can
    /// exercise input rules without loading a workspace.
    /// </summary>
    internal static void Validate(GenerateEqualsHashCodeParams @params)
    {
        if (@params.AllFiles)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var typeDecl = FindTypeDeclaration(root, typeName, @params.Line, @params.Column);
        ...
        if (!@params.ReplaceExisting)
        {
                ...
                (ImplementsIEquatable(typeSymbol) || HasCompatibleTypedEquals(typeSymbol)))
            {
                throw new RefactoringException(
                    ...
                    $"Type '{typeName}' already implements IEquatable<T> or already has a compatible typed Equals.");
            }
            ...
            if (@params.GenerateOperators && HasExistingEqualityOperators(typeSymbol))
            {
                throw new RefactoringException(
                    ...
                    $"Type '{typeName}' already declares operator == or operator !=.");
            }
            ...
            var pendingChanges = new List<PendingChange>
            {
                new()
                {
                    ...
                    BeforeSnippet = @params.ReplaceExisting
                        ? $"// Type '{typeName}' (replacing existing equality members)"
                        : $"// Type '{typeName}' (no Equals/GetHashCode)",
                    AfterSnippet = code
                }
            };
        ...
        var solution = await ApplyEqualsHashCodeToSolutionAsync(
    ...
    /// Equals/GetHashCode on every eligible
            ...
            while (true)
            {
                ...
                foreach (var typeDeclaration in CollectTypeDeclarations(root))
                {
                    ...
                    catch (RefactoringException)
                    {
                        ...
                        // / AlreadyImplementsIEquatable /
                        // AlreadyHasEqualityOperators / CallSuperOnObjectBase
                        // / CallSuperOnAbstractBase / uneditable /
            ...
            if (@params.Preview)
            {
                ...
                allPendingChanges.Add(new PendingChange
                {
                    ...
                    Description = generatedCount > 0
                        ...
                        : "Update Equals/GetHashCode generated in other files",
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            members = EqualityMemberCollector.CollectMembers(
                typeSymbol, requestedFields: null, @params.IncludeProperties, @params.IncludeInheritedMembers);
        }
        catch (RefactoringException)
        {
            return null;
        ...
        try
        {
            return await ApplyEqualsHashCodeToSolutionAsync(
                document.Project.Solution,
                document,
                typeDecl,
                typeSymbol,
    ...
    private static async Task<Solution> ApplyEqualsHashCodeToSolutionAsync(
        Solution solution,
        Document document,
        TypeDeclarationSyntax typeDecl,
        INamedTypeSymbol typeSymbol,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (replaceExisting)
        {
            ...
            targetTypeAnnotation = new SyntaxAnnotation("generate-equals-hashcode-target-type");
    ...
    /// <see cref="RemoveExistingEqualityMembersAcrossPartialsAsync"/>, recover
