...
public sealed class ToolRegistry
{
    ...
    public static ToolRegistry BuildDefault()
    {
        ...
        r.RegisterRefactoring<ExtractInterfaceOperation, ExtractInterfaceParams>(
            "extract-interface", "Extract an interface from a class; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; column (optional) picks the type whose identifier or declaration span covers that column when set with line; omitted keeps today's typeName + optional line pick; column without line keeps today's first-match after the typeName filter; separateFile writes {InterfaceName}.cs next to the source; sourceFile, typeName, and interfaceName are required when allFiles is omitted or false; allFiles (default false) extracts I{TypeName} for every eligible non-static type with extractable public members in every C# file into a sibling I{TypeName}.cs (sourceFile optional when true; cannot be combined with typeName, line, column, interfaceName, members, or targetFile); addInterfaceToType / preview remain valid with allFiles");
        r.RegisterRefactoring<ExtractBaseClassOperation, ExtractBaseClassParams>(
            "extract-base-class", "Extract a base class from common members; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; column (optional) picks the type whose identifier or declaration span covers that column when set with line; omitted keeps today's typeName + optional line pick; column without line keeps today's first-match after the typeName filter; separateFile writes {BaseClassName}.cs next to the source; sourceFile, typeName, baseClassName, and members are required when allFiles is omitted or false; allFiles (default false) extracts {TypeName}Base for every eligible non-static class with extractable members in every C# file into a sibling {TypeName}Base.cs (sourceFile optional when true; cannot be combined with typeName, line, column, baseClassName, members, or targetFile); makeAbstract / preview remain valid with allFiles");
        r.RegisterRefactoring<IntroduceParameterOperation, IntroduceParameterParams>(
            "introduce-parameter", "Introduce a method parameter from a local variable; column (optional) picks the matching local whose identifier or declaration span covers that column when set with line; omitted keeps today's start-line equality on the local declaration statement, then variableName FirstOrDefault; a continuation-line identifier is eligible when column is set; sourceFile, variableName, and line are required when allFiles is omitted or false; allFiles (default false) promotes every eligible local in every C# file (sourceFile optional when true; cannot be combined with variableName, line, or column); preview remains valid with allFiles");
        r.RegisterRefactoring<PullMembersUpOperation, PullMembersUpParams>(
            "pull-members-up", "Move selected members from a derived type onto an existing base class or interface; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; column (optional) picks the type whose identifier or declaration span covers that column when set with line; omitted keeps today's typeName + optional line pick; column without line keeps today's first-match after the typeName filter; sourceFile, typeName, and members are required when allFiles is omitted or false; allFiles (default false) pulls every pullable member from each eligible derived type onto its default target in every C# file (sourceFile optional when true; cannot be combined with typeName, line, column, members, or targetBaseType); makeAbstract / preview remain valid with allFiles");
        r.RegisterRefactoring<PushMembersDownOperation, PushMembersDownParams>(
            "push-members-down", "Move selected members from a base type down onto derived types; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; column (optional) picks the type whose identifier or declaration span covers that column when set with line; omitted keeps today's typeName + optional line pick; column without line keeps today's first-match after the typeName filter; sourceFile, typeName, and members are required when allFiles is omitted or false; allFiles (default false) pushes every pushable member from each eligible base type onto all direct derived types in every C# file (sourceFile optional when true; cannot be combined with typeName, line, column, members, or targetDerivedTypes); leaveAbstract / preview remain valid with allFiles");
        ...
        r.RegisterRefactoring<IntroduceFieldOperation, IntroduceFieldParams>(
            "introduce-field", "Turn a selected local variable or expression into a class field; sourceFile, startLine, startColumn, endLine, endColumn, and fieldName are required when allFiles is omitted or false; allFiles (default false) promotes every eligible local in every C# file (field named from that local; sourceFile optional when true; cannot be combined with startLine, startColumn, endLine, endColumn, or fieldName); isReadonly / isStatic / initializeInConstructor / replaceAll / preview remain valid with allFiles where they apply");
        r.RegisterRefactoring<SafeDeleteOperation, SafeDeleteParams>(
            "safe-delete", "Delete a selected symbol only when it has no remaining references; sourceFile and selection (startLine / startColumn / endLine / endColumn) are required when allFiles is omitted or false; allFiles (default false) deletes every eligible unused private (or local) declaration in every C# file (public / protected / internal skipped in bulk) (sourceFile optional when true; cannot be combined with startLine, startColumn, endLine, endColumn, or symbolName); preview remains valid with allFiles");
        ...
        r.RegisterRefactoring<RenameNamespaceOperation, RenameNamespaceParams>(
            "rename-namespace", "Rename a namespace across the solution, updating declarations and references; column (optional) picks the smallest namespace whose name or declaration span covers that column when set with line; omitted keeps today's namespaceName + optional line pick; column without line keeps today's omitted-line path; sourceFile and namespaceName are required when allFiles is omitted or false; allFiles (default false) renames every eligible top-level namespace declaration in every C# file to newName (sourceFile optional when true; cannot be combined with namespaceName, line, or column); newName remains required; updateFolders / preview remain valid with allFiles");

        // ── Refactoring: Inline (3) ───────────────────────────────────
        r.RegisterRefactoring<InlineVariableOperation, InlineVariableParams>(
            "inline-variable", "Inline a variable, replacing all references with its value; column (optional) picks the declaration whose identifier or declaration span covers that column; omitted keeps today's variableName + optional line start-line pick; sourceFile and variableName are required when allFiles is omitted or false; allFiles (default false) inlines every eligible local in every C# file (sourceFile optional when true; cannot be combined with variableName, line, or column); preview remains valid with allFiles");
        r.RegisterRefactoring<InlineMethodOperation, InlineMethodParams>(
            "inline-method", "Inline a method by replacing call sites with the method body; column (optional) picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName and/or line identifier start-line pick; sourceFile and methodName are required when allFiles is omitted or false; allFiles (default false) inlines every eligible method in every C# file (sourceFile optional when true; cannot be combined with methodName, line, column, or callSiteLocation); removeMethod and preview remain valid with allFiles");
        ...
        r.RegisterRefactoring<ChangeSignatureOperation, ChangeSignatureParams>(
            "change-signature", "Add, remove, or reorder method parameters; column (optional) picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName and/or line start-line pick; sourceFile and methodName are required when allFiles is omitted or false; allFiles (default false) applies the same parameters list to every eligible method in every C# file (sourceFile optional when true; cannot be combined with methodName, line, or column); parameters remains required; preview remains valid with allFiles");
        r.RegisterRefactoring<AddParameterOperation, AddParameterParams>(
            "add-parameter", "Add a named parameter to a method and update call sites; column (optional) picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName and/or line start-line pick; sourceFile and methodName are required when allFiles is omitted or false; allFiles (default false) adds the same parameterName/parameterType to every eligible method in every C# file (sourceFile optional when true; cannot be combined with methodName, line, or column); parameterName / parameterType remain required; defaultValue / position / updateOverrides / updateImplementations / preview remain valid with allFiles");
        r.RegisterRefactoring<RemoveParameterOperation, RemoveParameterParams>(
            "remove-parameter", "Remove a named parameter from a method and update call sites; column (optional) picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName and/or line start-line pick; sourceFile and methodName are required when allFiles is omitted or false; allFiles (default false) removes the same parameterName from every eligible method in every C# file (sourceFile optional when true; cannot be combined with methodName, line, or column); parameterName remains required; force / updateOverrides / updateImplementations / preview remain valid with allFiles");
        r.RegisterRefactoring<ReorderParametersOperation, ReorderParametersParams>(
            "reorder-parameters", "Reorder a method's parameters by a 0-based permutation and update call sites; column (optional) picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName and/or line start-line pick; sourceFile and methodName are required when allFiles is omitted or false; allFiles (default false) applies the same newOrder to every eligible method in every C# file (sourceFile optional when true; cannot be combined with methodName, line, or column); newOrder remains required; updateOverrides / updateImplementations / preview remain valid with allFiles");
        r.RegisterRefactoring<ChangeReturnTypeOperation, ChangeReturnTypeParams>(
            "change-return-type", "Change a method's return type and update return statements; column (optional) picks the smallest method whose identifier or declaration span covers that column; omitted keeps today's methodName and/or line start-line pick; sourceFile and methodName are required when allFiles is omitted or false; allFiles (default false) changes the return type of every eligible method in every C# file (sourceFile optional when true; cannot be combined with methodName, line, or column); newReturnType remains required; updateOverrides / updateImplementations / convertReturnStatements / preview remain valid with allFiles");
        ...
        r.RegisterRefactoring<ConvertToBlockBodyOperation, ConvertToBlockBodyParams>(
            "convert-to-block-body", "Convert an expression-bodied member to a block body; column (optional) picks the member whose identifier or declaration span covers that column on the given line; allFiles (default false) converts every eligible expression-bodied member in every C# file (sourceFile optional when true; cannot be combined with memberName, line, or column)");
