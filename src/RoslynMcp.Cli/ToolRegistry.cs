...
public sealed class ToolRegistry
{
    ...
    public static ToolRegistry BuildDefault()
    {
        ...
        r.RegisterRefactoring<ExtractInterfaceOperation, ExtractInterfaceParams>(
            "extract-interface", "Extract an interface from a class; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; separateFile writes {InterfaceName}.cs next to the source");
        r.RegisterRefactoring<ExtractBaseClassOperation, ExtractBaseClassParams>(
            "extract-base-class", "Extract a base class from common members; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; separateFile writes {BaseClassName}.cs next to the source");
        ...
        r.RegisterRefactoring<ConvertToAsyncOperation, ConvertToAsyncParams>(
            "convert-to-async", "Convert a synchronous method to async; column (optional) picks the method whose identifier or declaration span covers that column; updateCallers (default false) awaits already-async callers and skips sync callers that cannot legally await");
        ...
        r.RegisterRefactoring<GenerateEqualsHashCodeOperation, GenerateEqualsHashCodeParams>(
            "generate-equals-hashcode", "Generate Equals and GetHashCode overrides; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; implementIEquatable (default false) also adds IEquatable<T>; generateOperators (default false) also adds == and !=; replaceExisting (default false) replaces existing equality members instead of failing; useHashCodeCombine (default true) uses HashCode.Combine / builder instead of an unchecked prime-multiply GetHashCode; includeProperties (default true) includes readable properties as equality members; false uses instance fields only unless fields names a property; callSuper (default false) folds the immediate base type's Equals/GetHashCode into the generated methods; includeInheritedMembers (default false) also collects accessible instance fields/properties declared on base types");
        r.RegisterRefactoring<GenerateOverridesOperation, GenerateOverridesParams>(
            "generate-overrides", "Generate method, property/indexer, and event overrides from a base class; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; callBase (default true) emits base.Method() / base.Prop / base[i] for non-abstract virtuals (events always use empty add/remove); replaceExisting (default false) replaces already-overridden members instead of skipping them");
        r.RegisterRefactoring<GenerateToStringOperation, GenerateToStringParams>(
            "generate-tostring", "Generate a ToString override; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; format interpolated (default) or stringbuilder; includeProperties (default true) includes readable properties as ToString members; false uses instance fields only unless fields names a property; includeInheritedMembers (default false) also collects accessible instance fields/properties declared on base types; replaceExisting (default false) replaces an existing parameterless ToString instead of failing; callSuper (default false) folds the immediate base type's ToString into the generated override");
        r.RegisterRefactoring<ImplementInterfaceOperation, ImplementInterfaceParams>(
            "implement-interface", "Generate implementation stubs for an interface; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; throwNotImplemented (default true) throws NotImplementedException in stub bodies; replaceExisting (default false) replaces already-implemented interface members instead of failing; preview returns computed changes without applying");
        r.RegisterRefactoring<ImplementAbstractOperation, ImplementAbstractParams>(
            "implement-abstract", "Generate implementation stubs for unimplemented abstract members inherited by a class; line (optional) picks the type whose identifier or declaration span covers that line when several types share the name; omitted keeps today's typeName FirstOrDefault pick; throwNotImplemented (default true) throws NotImplementedException in stub bodies, false uses default-return / empty setter bodies; replaceExisting (default false) replaces already-implemented abstract members instead of failing; preview returns computed changes without applying");
        r.RegisterRefactoring<AddNullChecksOperation, AddNullChecksParams>(
            "add-null-checks", "Add null checks to method or constructor parameters; column (optional) picks the smallest method or constructor whose identifier or declaration span covers that column; omitted keeps today's methodName and optional line start-line pick");
