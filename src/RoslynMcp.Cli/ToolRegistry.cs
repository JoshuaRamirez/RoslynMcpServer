...
public sealed class ToolRegistry
{
    ...
    public static ToolRegistry BuildDefault()
    {
        ...
        r.RegisterRefactoring<ConvertAnonymousToClassOperation, ConvertAnonymousToClassParams>(
            "convert-anonymous-to-class", "Convert an anonymous type to a named class or record; column (optional) picks the anonymous creation whose span covers that column on the given line (exclusive-end; unique covering match); omitted keeps today's line pick; sourceFile, line, and newTypeName are required when allFiles is omitted or false; allFiles (default false) converts every distinct eligible anonymous-type shape in every C# file (type named from sanitized member names; sourceFile optional when true; cannot be combined with line, column, or newTypeName); asRecord / preview remain valid with allFiles");
